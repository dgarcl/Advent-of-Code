...
int main() {
    ...
    while (!singlecircuit) { // We repeat the following process until we have a single circuit
        ...
        // Update the last distance connected
        it++;
    }

    // Compute the product of the x coordinate of the last two junction boxes
    long long result = (long long) box1.getxcoord() * (long long) box2.getxcoord();

    cout << "If you multiply together the X coordinates of the last two junction boxes you need to connect, you get " << result << endl;
